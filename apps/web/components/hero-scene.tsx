...
export function HeroScene() {
  return (
    <div className="hs-scene ink-panel pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden>
      ...
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_18%_22%,rgba(157,78,221,0.35),transparent_45%),radial-gradient(circle_at_78%_18%,rgba(216,207,227,0.3),transparent_42%),radial-gradient(circle_at_60%_85%,rgba(157,78,221,0.18),transparent_45%)]" />
      ...
      <div className="hs-map absolute inset-0 z-10">
        <svg className="h-full w-full opacity-[0.16]" viewBox="0 0 1200 500" preserveAspectRatio="xMidYMid slice" role="presentation">
          <g fill="none" stroke="#9D4EDD" strokeWidth="1">
            {[-3, -2, -1, 0, 1, 2, 3].map((k) => (
      ...
      <div className={`${flightBand} z-20`}>
        ...
        {routeStops.map((stop) => (
          ...
          >
            <span
              className="hs-pin-ring absolute inset-0 rounded-full bg-brand-400/45"
              style={{ animationDelay: stop.delay }}
            />
            <span className="absolute inset-0 rounded-full bg-brand-400/90 shadow-[0_0_10px_rgba(157,78,221,0.8)]" />
          </span>
        ))}
      </div>
      ...
      <svg
        ...
        role="presentation"
      >
        <path d="M0 320V150q180-96 360-30t300-52 240 44 300-24v232Z" fill="#5A189A" opacity="0.32" />
        <path d="M0 320V212q220-84 420-18t340-40 440 40v126Z" fill="#5A189A" opacity="0.34" />
        <path d="M0 320v-64q260-56 480 4t360-26 360 30v56Z" fill="#240046" opacity="0.45" />
      </svg>

      {/* 5 — scrim: keeps the copy above at full contrast wherever the plane passes */}
      <div className="absolute inset-0 z-40 bg-[radial-gradient(ellipse_60%_50%_at_50%_46%,rgba(36,0,70,0.55),transparent_75%)]" />
    </div>
  );
}
    ...
    <svg viewBox="0 0 64 64" className="h-full w-full" role="presentation">
      <defs>
        <linearGradient id="hsPlaneBody" x1="0" y1="0" x2="0" y2="1">
          ...
          <stop offset="52%" stopColor="#FFE3C2" />
